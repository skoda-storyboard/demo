...
test('emits the expected Tags block for a multi-tag selection (one row per tag)', () => {
  ...
  assert.equal(
    html,
    '<table><tbody><tr><th>Tags</th></tr>'
    + '<tr><td><a href="/en/tag/model/epiq/">Epiq</a></td></tr>'
    + '<tr><td><a href="/en/tag/years/2026/">2026</a></td></tr>'
    + '</tbody></table>',
  );
});
...
test('an empty selection yields just the header row (no tag rows)', () => {
  assert.equal(
    buildTagsBlockHTML([]),
    '<table><tbody><tr><th>Tags</th></tr></tbody></table>',
  );
});
