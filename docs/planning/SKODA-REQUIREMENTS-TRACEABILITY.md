...
|---|--------|---------|--------|
| QA-F1 | **SKODA-605** (E06, M1) | Models/Series rail + "All" links are absolute `skoda-storyboard.com` URLs (leave the demo) → transformer link-rewrite | **Code done 2026-09-25** (`skoda-links.js`, all 16 importers; demo-page allow-list, the rest stay absolute per D-3 (b), amended AC): 0 absolute demo links on 30 re-imported pages (was 45). 2 pages published; the rest are held by 508/603/607/801a/208 (see ticket). Rails are index-driven now, so F1 no longer reproduces there |
