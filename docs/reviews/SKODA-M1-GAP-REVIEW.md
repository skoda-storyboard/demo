...
4. **Remaining Must work ≈ 57.5 SP** in the build window, plus **≈ 11 SP** of QA gates in the dry-run window.
   - Build-window capacity is **≈ 28 dev-days**, which is **46–75 SP (mid ≈ 57)** at the OVERVIEW ratio of 0.37–0.61
     AI-day/SP.
   - So **Must fills 100% of mid-velocity capacity.**
   - About 8 SP of Must (603 import runs, 605/606, 701) is planned as agent-executed under human review. If that
     holds, human demand is ≈ 24 dev-days, leaving **≈ 3–4 days of slack**. If the agents under-deliver, slack is
     zero. §9–§10 give the overflow order.
...
| Story detail | 21 (20 unique) | `template-layout-article`: `sow-editor` + `skoda-offset` + `skoda-carousel-widget` only | 801, **801a**, **219**, 502, 204, 205, 604 |
...
| | **In-body image carousel** (21/21) | 🔴 | **219**, **801a** | demo-blocking (text-only stories) |
| | Media Box → downloads + cart hook (21/21) | 🔴 mapping (importer drops it) | **801a**, 502, 505a | demo-visible |
| | Vimeo embeds (2/21) | 🔴 mapping | **801a**, 204 | demo-visible |
...
| 9 | 212 | FAIL | Rail cell widths are 270/216/294px at 1280/768/375 vs source 281/230/338 (22.5% / 30% / 90%, 20px gap, 1248px cap) | demo-visible | Fix with 219 (same block) |
...
| Story sidebar (STO-D07) | M2 | Traceability §2.E |
...
| 801 | In progress | Widget mapping wrong for `skoda-carousel-widget` | Corrected. The M1 fidelity work is split into **801a** |
...
| G-02 | `skoda-carousel-widget` unspecced, unbuilt, dropped (21/21 stories) | live markup; importer | demo-blocking | **219 + 801a** |
| G-03 | Story importer drops the Media Box (21/21) and Vimeo (2/21) | `skoda-story-cleanup.js` drop list | demo-visible | **801a** |
...
### Must (build window, ≈ 57.5 SP): the demo fails without these
...
805b option B, Peaq children (3) · 216 `.sb-gallery` (3) · 215 share (1) · 705 button variant (1).
...
4. **Overflow order:** the spare days (vijay ≈ 2.4 d, saran ≈ 0.7 d) first absorb Lars's overflow (import runs), then
...
| Surface | Owner | Rule |
|---|---|---|
| `blocks/carousel` | vijay | 212 fix → 219, in that order |
| `blocks/story-rail` | vijay | hide-empty rails are built in 208 only; 608 just supplies the rows |
| `blocks/footer` | vijay | 305 → 306 |
| `blocks/header` | vijay | 302, then 303 |
| `blocks/cards`, `card-teaser`, global `styles/*` | Lars | the 213/217/218 WIP; no other global CSS edits until it merges (W1). 805a/207 `cards` CSS waits for that merge |
...
| Shared `transformers/skoda-page-cleanup.js` (used by all importers), the new link-policy step, and all generated `*.bundle.js` | Lars → saran | 605 → 606 merge **before** the 607/801a PRs open. 609 is implemented *inside* the shared cleanup (saran, after 606). Regenerate bundles only after rebasing |
| `import-story-detail.js`, `skoda-story-cleanup.js` | vijay | 801a (Lars hands over 801) |
...
| **W1** | Mon 28 Sep – Thu 1 Oct | 501 masters-only → 506 pre-conditioning gate → **609** containment + alias redirect (agent-run, saran reviews; starts once 606 merges Tue, merged by Thu 1 Oct) → start 505a | **Mon: pin the 219 authored block-table shape with Lars**, so that 801a can start independently. Then 305 MR footer → 306; 302 nav; 212 rail fix → start 219 rendering | **607** PR importer + template (Buzzsprout, related rail, downloads) → **805a** hubs (805a PR closes after 609 merges) | 602 DA push/publish; 605/606 (agent) merged by Tue; 603 tracker + first import (home, listings) |
...
| **W2** | Fri 2 – Tue 6 Oct | finish 505a → 505b (live DAM) → **608** image/video items + 402 fix | **801a** story importer first (against the pinned 219 table) → finish 219 rendering → 208 blocks + hide-empty rails | Fri 2: **207** hubs (D-4). Budget spent | Re-import home + listings with 609 applied; 603 imports: 5 PRs, 5 models, 3 hubs, corpus; **the 21 stories are imported after 801a merges (target Mon 5)** |
...
| vijay | 15.5: 204, 212 fix, 302, 305, 306, 219, 801a, 208 | 7.6 of 10 | 76% |
...
| **Total** | **57.5** (= Must) | ≈ 24.3 human dev-days | of ≈ 28 |

The remaining margin (vijay ≈ 2.4 d, saran ≈ 0.7 d) goes in the risk-statement order: first Lars's overflow, then
...
1. **Create issues** from the new ticket files, then add them to Project #1 with milestone M1:
   - [SKODA-219](../tickets/tickets/SKODA-219.md): P0, 3 SP, vijay
   - [SKODA-801a](../tickets/tickets/SKODA-801a.md): P0, 3 SP, vijay, parent #48
...
2. **Assign:**
   ...
   | vijay | 204, 212 fix, 302, 305, 306, 219, 801a, 208, 703 | 805c, 604, 303 |
   | extra resource | 607, 805a, 207 | – |
   | Lars | 213, 217, 218, 602, 603, 605, 606, 701, 704, 707 | – |

   - Unowned Could: 805b, 216, 215, 705.
3. **Milestones:**
