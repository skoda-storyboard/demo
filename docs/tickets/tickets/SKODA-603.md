...
| **W1** | 11 corpus PRs; re-push the 5 set PRs + `/en` with 610; re-import the 3 stale stories (plates, Peaq record, Octavia) | 610, 506; the `quote` and `promo-box` contracts in the importers |
...
| **W2c** | 5 set + 11 corpus models | the 208 importer fix (`hero` → Hero Image, no `subheading`, contracts) |
| **W3** | 5 + 12 series hubs (`Cards (overlay, tiles)`); 4 + 12 press kits | 207, 221, 805a, 805c |
...
re-QA, not a re-import. Pages whose pending blocks have a **broken** fallback stay preview-only unless they're
explicitly approved.
...
- **The block check found 31 pages with errors in the current importer output:**
  - **`hero` has no block on `main`.** `parsers/hero.js` and `hero-banner.js` emit `Hero`; only `hero-image`
    exists. Affected: models, series, listings, and stale story output.
