...
| **Titles are cleaned at import** | `skoda-metadata.js` strips the source's " - Škoda Storyboard" SEO suffix from the Metadata `Title` (SKODA-610); `loadQueryIndex()` strips it again at render time. Don't re-add it. The rule is kept in sync by a test across `skoda-metadata.js`, `skoda-metadata-extract.mjs` and `scripts/query-index.js`. |
| **`media:apply` is all-or-nothing** | One unresolved image (e.g. a Vimeo poster `i.vimeocdn.com` thumbnail) blocks the whole `--pages` batch; run it per page to see which pages are blocked. |
